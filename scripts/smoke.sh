...
# With per-agent clients the profile provisions perf-agent-0..9 only — no
# shared perf-test-client — so the smoke test authenticates and audiences as
# agent 0, exactly like k6 agent 0 does. SMOKE_AUDIENCE still overrides for
# single-client profiles.
SMOKE_CLIENT="${CLIENT_ID}"
SMOKE_AUD="${SMOKE_AUDIENCE:-${SUBJECT_AUDIENCE}}"
if [[ "${PER_AGENT_CLIENTS:-true}" == "true" ]]; then
  SMOKE_CLIENT="perf-agent-0"
  SMOKE_AUD="${SMOKE_AUDIENCE:-perf-agent-0}"
fi
subject_token="$(python3 - "${SUBJECT_SIGNING_KEY_FILE}" "${SUBJECT_ISSUER}" "${SMOKE_AUD}" "${SUBJECT_USER}" <<'PYEOF'
...
# No actor_token: the act claim's sub is the authenticated client id, built
# server-side by the access token mapping's OGNL expression.

if [[ "${CLIENT_AUTH_METHOD:-client_secret_basic}" == "client_secret_post" ]]; then
  curl_args+=(--data-urlencode "client_id=${SMOKE_CLIENT}" --data-urlencode "client_secret=${CLIENT_SECRET}")
else
  curl_args+=(--user "${SMOKE_CLIENT}:${CLIENT_SECRET}")
fi
